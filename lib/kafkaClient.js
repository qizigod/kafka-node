...
KafkaClient.prototype.createBroker = function (host, port, longpolling) {
  ...
  socket.on('close', function (hadError) {
    ...
    if (!hadError && self.closing) {
      logger.debug(`clearing ${this.addr} callback queue`);
      self.clearCallbackQueue(this, new Error(`${this.addr} has closed`));
    } else {
...
KafkaClient.prototype.sendRequest = function (request, callback) {
  ...
  const sendToBroker = async.ensureAsync((payload, leader, callback) => {
    ...
    const compress = (payload, leader, callback) => {
      assert(!_.isEmpty(broker.apiSupport), 'apiSupport is empty');
      const magic = broker.apiSupport[request.type].usable === 2 ? 1 : 0;
      ...
      function buildRequest (payload, cb) {
        ...
        const innerSet = encodeMessageSet(payload.messages, magic);
    ...
    const writeRequestData = (callback) => {
      const correlationId = this.nextId();
    ...
    async.series([
      callback => {
        if (request.type === 'produce') {
          ...
          compress(payload, leader, callback);
          return;
        }
        callback(null);
      },
      callback => {
        writeRequestData(callback);
      }
    ], callback);
  });
